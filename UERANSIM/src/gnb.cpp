...
static nr::gnb::GnbConfig *ReadConfigYaml()
{
    ...
    if (yaml::HasField(config, "phyLocation"))
    {
        ...
        result->phyLocation.x = yaml::GetInt32(config, "x", 0, 300);  // Default to 0.0f if not provided
        result->phyLocation.y = yaml::GetInt32(config, "y", 0, 300);
        result->phyLocation.z = yaml::GetInt32(config, "z", 0, 300);
        // std::cout << result->phyLocation.x << std::endl;
