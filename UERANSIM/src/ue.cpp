...
static nr::ue::UeConfig *ReadConfigYaml()
{
    ...
    // added by Henok
        // Check if the 'phyLocation' field exists in the config file
    if (yaml::HasField(config, "phyLocation"))
    {
        // auto phyLocation = config["phyLocation"];
        result->simpos.x = yaml::GetInt32(config, "x", 0, 300);  // Default to 0.0f if not provided
        result->simpos.y = yaml::GetInt32(config, "y", 0, 300);
        result->simpos.z = yaml::GetInt32(config, "z", 0, 300);
        result->home.x = yaml::GetInt32(config, "x", 0, 300);  // the starting position is home
        result->home.y = yaml::GetInt32(config, "y", 0, 300);
        result->home.z = yaml::GetInt32(config, "z", 0, 300);
        // std::cout << result->simpos.x << std::endl;
        
    }

    if (yaml::HasField(config, "work"))
    {
        // auto phyLocation = config["phyLocation"];
        result->work.x = yaml::GetInt32(config, "x1", 0, 300);  // Default to 0.0f if not provided
        result->work.y = yaml::GetInt32(config, "y1", 0, 300);
        result->work.z = yaml::GetInt32(config, "z1", 0, 300);
...
static nr::ue::UeConfig *GetConfigByUe(int ueIndex)
{
    ...
    c->simpos = g_refConfig->simpos;   // added by Henok
    c->home = g_refConfig->home;   // added by Henok
    c->work = g_refConfig->work;   // added by Henok
