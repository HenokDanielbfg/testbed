...
// //////////////////////////////////////////////////////////////////////////////////////////for prometheus
var GNBCOUNTER = prometheus.NewGaugeVec(
	prometheus.GaugeOpts{
		Name: "number_of_gNbs",
		Help: "Tracks the number of active gNbs",
	},
	[]string{})

func Prometheus() {
	prometheus.MustRegister(GNBCOUNTER)
	metricsAddr := ":8100"
	...
	go func() {
		// defer a.wg.Done()
		logger.Log.Infof("Starting AMF metrics server (listening on %s)", metricsAddr)
	...
	// a.wg.Add(1)
	// go metricsServer.listenShutdownEvent()
}
...
func SendNGSetupResponse(ran *context.AmfRan) {
	...
	GNBCOUNTER.WithLabelValues().Inc()
